...
function getFutureRiskPreparationNew(symbol){
	var typeDateFormat = '%Y-%m-%d %A';
	jQuery.getJSON('/future/futureRiskPreparation.do?symbol='+symbol+'&type=0', function(data) {
		var all = data.allData ;
		var weekly = data.weekData ;
		...
		var option = {
			...
			tooltip: {
				...
				formatter: function (params) {
					...
					var date = new Date(parseFloat(temp[0]));
					var a=temp[1];

					var show =date.format('yyyy-MM-dd hh:mm:ss')+'<br/>';
					show+=params[0].seriesName+'：'+parseFloat(a).toFixed(2)+'<br/>';
					return show;
				}
			},
			legend: {
				data:[get$("clawbackall")],
				top:'bottom'
			},
			xAxis: {
				type:'time',

...
function klinetradeTurnoverValue(type,event){
    ...
    if(document.getElementById("klineuserBalance")!=null){
        userBalance = document.getElementById("klineuserBalance").value;
        userBalanceBorrowAll = accAdd_z(userBalance, getCanBorrowAmount(klinetradeType));
    }
	if(klinetradeType ==0){
		var klinetradeTurnover = klinetradeAmount*klinetradeCnyPrice;
		if(Number(userBalanceBorrowAll) >= 0 && Number(userBalanceBorrowAll) < Number(klinetradeTurnover)){
			klinealertTipsSpan(buytradejs1);
		}else{
			klineclearTipsSpan();
		}
        showNeedBorrowAmount(klinesymbol, klinetradeType, userBalance, klinetradeTurnover);
	}else{
		if(Number(userBalanceBorrowAll) >= 0 && Number(userBalanceBorrowAll) < Number(klinetradeAmount)){
			//newCoinLabel
			if(klinesymbol == 0){
        ...
        showNeedBorrowAmount(klinesymbol, klinetradeType, userBalance, klinetradeAmount);
	}
}
var scheck = 1;
function klinesubmitTradeBtcForm(){
	...
	var limited = 0;
	var klineuserBalance = document.getElementById("klineuserBalance").value;
	var klineopenBorrow = (klinetradeType == 0 ? jQuery('#borrowCnySwitch').val() == 1 : jQuery('#borrowBtcSwitch').val() == 1);
	var klinelimitedMoney = Number(document.getElementById("klinelimitedMoney").value);
	var isOpenBorrowAndNeed = false;
	if (!islimited) {
		isOpenBorrowAndNeed = klineopenBorrow && isNeedBorrowAndTrade(klinetradeType, 0, klinetradeCnyPrice, klinetradeAmount, klinesymbol);
	} else {
		isOpenBorrowAndNeed = klineopenBorrow && isNeedBorrowAndTrade(klinetradeType, 1, klinetradeCnyPrice, klinelimitedMoney, klinesymbol);
	}

	if (isOpenBorrowAndNeed) {
		if (klinetradeType == 0) {
			klineuserBalance = accAdd(Number(klineuserBalance), Number(jQuery('#cnyCanBorrow').val()));
		} else {
			klineuserBalance = accAdd(Number(klineuserBalance), Number(jQuery('#btcCanBorrow').val()));
		}
	}
	...
	if(!islimited){
		if(klinetradeType ==0){
			var klinetradeTurnover = klinetradeAmount*klinetradeCnyPrice;
			if(Number(klineuserBalance) <  Number(klinetradeTurnover)){
				klinealertTipsSpan(buytradejs1);
				return;
			}else{
				klineclearTipsSpan();
			...
			if(Number(klineuserBalance) <  Number(klinetradeAmount)){
				//newCoinLabel
				if(klinesymbol == 0){
					klinealertTipsSpan(buytradejs2);
		...
		if(klinetradeType ==0){
			if(Number(klineuserBalance) <  klinelimitedMoney){
				klinealertTipsSpan(buytradejs1);
				return;
			}else{
				klineclearTipsSpan();
	...
	var url = "";
	if (isOpenBorrowAndNeed) {
		url = "/trade/borrowAndTrade.do?random=" + Math.round(Math.random() * 100);
		param.tradeType = Number(klinetradeType) + 1;
	} else {
		if (klinetradeType == 0) {
    ...
    if(!islimited){
        var callback = {okBack:function(){
            klinesubmitTradeBtcForm_post(url,param,klinetradeType,klinesymbol,isOpenBorrowAndNeed);
        },noBack:void(0)};
...
function klinesubmitTradeBtcForm_post(url,param,klinetradeType,klinesymbol,isOpenBorrowAndNeed){
    scheck = 2;
    ...
    jQuery.post(url,param,function(data){
        ...
        if(result!=null){
			...
			if (isOpenBorrowAndNeed) {
				refreshBorrowAndTradeData(klinesymbol, klinetradeType);
				var btnId = "klinebtnA";
				if (result.resultCode < -20000) {
					result.resultCode = result.resultCode + 20000;
					waitingStationStatic(btnId, ordering);
					klinealertTipsSpan(borrowAndTradeError);
					setTimeout(function () {
						clearWaitingStation(btnId, sellout);
					}, 500);
					...
					if (res == -7) {
						klinealertTipsSpan(get$("margintrading11"));
					} else if (res == -15) {
						klinealertTipsSpan(youHaveOtcOrder);
					} else if (res == -17) {
						experienceBack();
					} else {
						klinealertTipsSpan(borrowFail);
					}
					setTimeout(function () {
						clearWaitingStation(btnId, sellout);
					}, 500);
            ...
            if(result.resultCode == -1){
				...
				klinealertTipsSpan(get$("reBackMoney"));
				...
				tradeTurnoverValue_borrow(klinesymbol, klinetradeType);
            }else if(result.resultCode == 2){
	...
	// 一键杠杆
	if (isOpenBorrowAndNeed) {
		waitingStationStatic("klinebtnA", borrowing);
	}
}
...
function klinesummoneyValue(event){
    ...
    var klineuserBalance = 0, klineuserBalanceBorrowAll = -1;
    if(document.getElementById("klineuserBalance")!=null){
        klineuserBalance = document.getElementById("klineuserBalance").value;
        klineuserBalanceBorrowAll = accAdd_z(klineuserBalance, getCanBorrowAmount(klinetradeType));
    }
	if(klinetradeType ==0){
		if(Number(klineuserBalanceBorrowAll) >= 0 && Number(klineuserBalanceBorrowAll) < Number(klinetradeTurnover)){
			klinealertTipsSpan(buytradejs1);
		}else{
			klineclearTipsSpan();
		}
        showNeedBorrowAmount(klinesymbol, klinetradeType, klineuserBalance, klinetradeTurnover);
	}else{
		if(Number(klineuserBalanceBorrowAll) >= 0 && Number(klineuserBalanceBorrowAll) < Number(klinetradeAmount)){
			//newCoinLabel
			if(klinesymbol == 0){
				klinealertTipsSpan(buytradejs2);
			}else if(klinesymbol == 1){
        ...
        showNeedBorrowAmount(klinesymbol, klinetradeType, klineuserBalance, klinetradeAmount);
	}
}
...
function klinelimitedSummoneyValue(){
	...
	if(klinetradeType == 0){
        ...
        var userBalanceBorrowAll = accAdd_z(klineuserBalance, getCanBorrowAmount(klinetradeType));
        if(Number(money) > Number(userBalanceBorrowAll)){
			klinealertTipsSpan(buytradejs1);
		}else{
            klineclearTipsSpan();
        }
        showNeedBorrowAmount(klinesymbol, klinetradeType, klineuserBalance, money);
	}else{
		var coinBalance = Number(document.getElementById("klineuserCoinBalance").value);
        var coinBalanceBorrowAll = accAdd_z(coinBalance, getCanBorrowAmount(klinetradeType));
        ...
        showNeedBorrowAmount(klinesymbol, klinetradeType, coinBalance, money);
	}
}
...
function klineonPortion(portion){
	var klinetradeType = document.getElementById("klinetradeType").value;
	// 获得融资融币开关的状态
    var borrowSwitch = getBorrowSwitch(klinetradeType);
	if(klinetradeType == 0){
		var money = Number(document.getElementById("userCnyBalance").value);
        if(borrowSwitch==1) {
            money = accAdd_z(money, getCanBorrowAmount(klinetradeType));
        }
